...
content = content.replace(/showToast\(`U pastruan \$\{totalCleaned\} rrjeshta bosh kudo\.`\)/g, "showToast(t(`U pastruan ${totalCleaned} rrjeshta bosh kudo.`, `Cleared ${totalCleaned} empty rows everywhere.`))");
...
for (const [sq, en] of Object.entries(otherTranslations)) {
  const safeSq = sq.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const regex = new RegExp(`>\\s*${safeSq}\\s*<`, 'g');
  content = content.replace(regex, `>{t('${sq}', '${en}')}<`);
  
  const regexTitle = new RegExp(`title=["']${safeSq}["']`, 'g');
